...
wss = list(15)
for (k in 2:15){ 
  wss[k] = sum(kproto(cluster_frame, k, iter.max = 1000,nstart = 3)$tot.withinss)
}
...
kmodes_fit = kmodes(cluster_frame, 4,iter.max = 1000000)
kmodes_fit$size
kmodes_fit$modes


kproto_fit = kproto(cluster_frame, 4,iter.max = 1000000,nstart = 3)
kproto_fit$size
kproto_fit$centers
...
cluster_plot$cluster <- as.factor(kproto_fit$cluster)
...
sys_tree <- subset(cluster_plot, select=-c(dia_mean,mult,riagendr,diq010,mult,asian,black,white,hispanic,ridageyr,bpq040a, bpq020))
