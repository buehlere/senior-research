...
bpsys_linear2 = lm(sys_mean~bmxarmc + log(bmxsad1) + bmxwaist + riagendr + ridageyr + I(ridageyr^2) + log(lbdldl) + log(lbxtc) + I(lbxglu^(1/3)) + hispanic + black + ridageyr:white + ridageyr:lbdldl + bmxarmc:ridageyr, sys_lin)
...
### MSE frame and plot for dia
  bar_dia <- mse %>% select(first_dia_mse, predict_dia_mse) %>% summarise(first_dia_mse = mean(first_dia_mse), predict_dia_mse = mean(predict_dia_mse))
...
ggplot(mse, aes(predicted_dia,true_dia))+
    geom_point() + geom_abline(aes(intercept=0, slope=1)) + geom_smooth(method="lm")
```
