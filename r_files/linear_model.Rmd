...
table1 <- export_summs(bpdia_linear2, scale = TRUE)
...
ggplot(mse, aes(dia_lin,true_dia))+
    geom_point() + geom_abline(aes(intercept=0, slope=1, col = "red")) + geom_smooth(method="lm")  + labs( 
   x="Predicted Diastolic", y="True Diastolic") + theme_minimal()

ggplot(mse, aes(sys_lin,true_sys))+
    geom_point() + geom_abline(aes(intercept=0, slope=1, col = "red")) + geom_smooth(method="lm") + labs( 
